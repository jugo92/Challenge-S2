...
const buttons = {
    ...
    "register" : {
        "content" : "VALIDER MON INSCRIPTION",
        "color" : "button-details",
        "action" : "/register"
    },
    "getSocialButton" : {
        "content" : "Get Social",
        "color" : "button-invalid",
...
<template>
    <router-link v-if="redirect" :to="name ? buttons[name].action : action">
        <button :class="name ? buttons[name].color : colors[color]" class="text-white font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2 w-full">
            {{ name ? buttons[name].content : content }}
        </button>
    </router-link>
    <button v-else :class="name ? buttons[name].color : colors[color]" class="text-white font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2 w-full" type="button">
        {{ name ? buttons[name].content : content }}
    </button>
</template>
<!--  Example of button:-->
<!--<Button content="Nouveau Produit" color="green" @click="goToLogin"/>-->
<!--<Button name="login"/>-->
<!--<Button name="login" redirect/>-->
<!--<Button content="Se Connecter" color="yellow" redirect action="/login"/>action="/route" />&ndash;&gt;-->
