        ...
        <form enctype="multipart/form-data" :class="{['grid grid-cols-2'] : format==='row'}">
            <div v-for="(field, index) in formFields" :key="index" :class="{['px-4'] : format==='row', ['flex flex-col w-full'] : format==='column'}" class="mb-4">
                    <template v-if="field.type !== 'button'">
                        ...
                        <template v-else-if="field.type === 'file' && !showField(field)">
                            <div class="mt-4">
                                <label @click="deleteImage(formFields)" class="text-red-600 underline cursor-pointer">Supprimer l'image</label><br>
                                <div class="inline-block mr-4">
                                    <img :src="getFileUrl(field.value)" class="w-32 h-32 object-cover" />
                                </div>
                            </div>
                        </template>
                        ...
                        <template v-else-if="field.type === 'textarea' && showField(field)" @input="callChangeHandlers(field.changeHandlers)">
                            <textarea :id="field.name" :name="field.name" v-model="field.value" :required="field.required" :placeholder="field.placeholder" rows="5" class="form form-input-textarea"></textarea>
                        </template>
