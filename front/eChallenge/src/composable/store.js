...
const store = createStore({
  ...
  mutations: {
    ...
    setProductDetails(state, productDetails) {
      ...
      const productIndex = state.products.findIndex(p => p._id === productDetails.product._id);
      if (productIndex !== -1) {
        state.products[productIndex].receiveNotifications = productDetails.product.receiveNotifications;
      }
    },

    setProductNotification(state, { productId, receiveNotifications }) {
      if (state.productNotifications[productId]) {
        state.productNotifications[productId].receiveNotifications = receiveNotifications;
      } else {
    ...
    setFilteredProperties(state, products) {
      state.filteredProperties = {
        ...
        Systeme: Array.from(new Set(products.map(product => product.operatingSystem))),
        ...
        Marque: Array.from(new Set(products.map(product => product.Marque.name))),
        Category: Array.from(new Set(products.map(product => product.Category.name))),
      };
    },
  ...
  actions: {
    ...
    async updateProductNotification({ commit, rootState }, { productId, receiveNotifications }) {
      if (rootState.isLoggedIn) {
        try {
          const response = await fetch(`http://localhost:3000/api/products/${productId}/notify`, {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
