...
<template>
    ...
    <span class="text-xs flex justify-start basis-1/3">
      <span class="flex flex-col items-center" @click="isOpen = !isOpen">
    ...
    <nav class="fixed inset-x-0 w-2/3 md:w-2/6 xl:w-1/6 h-full px-6 py-4 bg-pink-300" @click.stop="preventClose" :class="{ ['translate-x-0 transition-all duration-700 ease-in-out active']: isOpen, ['-translate-x-full transition-all duration-700 ease-in-out']: !isOpen }">
        <ul class="flex flex-col justify-center gap-4 transition-all duration-1000 ease-in-out">
            <li>
                <router-link to="/login" class="text-xs flex items-center gap-5">
                    <Icon icon="mdi:account" height="36" />
                    Me connecter
                </router-link>
            </li>
            <p>Rayons</p>
            <li>
        <span class="text-xs flex items-center gap-5">
          <Icon icon="mdi:discount" height="36" />
          Bon plans
        </span>
            </li>
            <p>Catégorie</p>
            <ul>
                <li v-for="menu in menuItems" :key="menu.id">
                    <a :href="menu.link" :title="menu.link">
        <span class="text-xs flex items-center gap-5">
          <Icon :icon="menu.icon" height="36" />
          {{ menu.name }}
        </span>
                    </a>
                </li>
...
<style scoped>
/* Ajoutez des styles CSS pour votre navbar ici */
.active {
